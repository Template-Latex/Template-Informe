...
class MLP:
    ...
    def train(self):
        print('Training with activator {0}'.format(model.ACTIVATOR))
        """training"""
...
if len(sys.argv) != 2:
    raise Exception('Numero de parametros incorrecto')
modelo = sys.argv[1].replace('--', '')

# Obtiene el ultimo activador usado
activador = ''
optimizador = ''
f = open('tf-MLP/mlp/mlp_model.py', 'r')
for i in f:
    if 'ACTIVATOR' in i and activador == '':
        activador = i.strip().split(' = ')[1].replace("'", '').split('#')[0].strip()
    if 'OPTIMIZER' in i and optimizador == '':
        optimizador = i.strip().split(' = ')[1].replace("'", '').split('#')[0].strip()
f.close()
if activador == '':
    raise Exception('No se encontro el activador')
if optimizador == '':
    raise Exception('No se encuentra el optimizador')
...
def genloss(f, o):
    ...
    efile = ''
    for i in d:
        if 'events.out.tfevents' in i and '.PPIZARRORPC' in i:
            efile = f + i
            break
    if efile == '':
        raise Exception('Archivo de eventos no encontrado en {0}'.format(f))
    # Carga el accuracy
    d = os.listdir(f + 'eval/')
    accfile = ''
    for i in d:
    ...
    # Obtiene el accuracy
    accuracy = 0
    for e in tf.train.summary_iterator(accfile):
    ...
    of = open(o, 'w')
    for e in tf.train.summary_iterator(efile):
        loss = 0
        # accuracy = 0
        for v in e.summary.value:
            if v.tag == 'loss':
            ...
            #if v.tag == 'accu':
            #    accuracy = v.simple_value
        if loss != 0 and accuracy != 0:
            of.write('{0}\t{1}\t{2}\n'.format(e.step, loss, accuracy))


# Obtiene el learning rate
def getlearning(p, m):
    f = p + 'tf-MLP/tools/'
    if m == 'mnist':
        f += 'train_mnist.py'
    else:
        f += 'train_quickdraw.py'
    of = open(f, 'r')
    for i in of:
        if 'learning_rate' in i:
            j = i.split(':')[1].split(',')[0].strip()
            return j

lr = getlearning(dir_path, modelo)

# Genera el archivo
lossfile = 'loss-{0}-{1}-{2}-{3}.txt'.format(modelo, optimizador, activador, lr)

# Tipo de modelo
if modelo == 'mnist':
    genloss(dir_path + 'MNIST-5000/model/', lossfile)
elif modelo == 'quick':
    genloss(dir_path + 'QuickDraw-Animals/model/', lossfile)
else:
