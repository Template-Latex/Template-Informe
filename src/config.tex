...
\def\captionlrmarginmc {1}         % Mar. izq/der leyenda dentro de columnas [cm]
...
\def\captiontbmargintable {7}      % Margen sup/inf de la leyenda en tab. [pt]
...
\def\marginaligncapttop {-0.8}     % Margen superior caption en align [cm]
...
\def\margineqncaptiontop {-0.75}   % Margen superior caption ecuación [cm]
...
\def\margingatheredcaptbottom{0.1} % Margen inf. caption en gathered [cm]
...
\def\marginfloatimages {-13}       % Margen sup. fig. insertimageleft/right [pt]
\def\marginfootnote {10}           % Margen derecho footnote [pt]
\def\margingatherbottom {-0.3}     % Margen inferior entorno gather [cm]
\def\margingatheredbottom {-0.2}   % Margen inf. entorno gathered [cm]
